...
bool Consensus_Group::is_in_Consensus_Group(std::string ip, uint32_t port)
{
    ...
    {
        if (miner_list[i].ip == ip && miner_list[i].port == port)
            return true;
    }
...
bool Consensus_Group::is_in_future_consensus_round(std::string ip, uint32_t port, int round)
{
    ...
    {
        if (tmp.miner[i].ip == ip && tmp.miner[i].port == port)
            return true;
    }
    return false;
}
...
miner_info *Consensus_Group::get_member_in_future_consensus_round(std::string ip, uint32_t port, int round)
{
    ...
    {
        if (future_info.find(round)->second.miner[i].ip == ip && future_info.find(round)->second.miner[i].port == port)
            return &future_info.find(round)->second.miner[i];
    }
...
void Consensus_Group::print_consensus_info()
{
    ...
    printf("\n=============== [CONSENSUS GROUP INFO: ]   Round_now:  %d     Blocks:  %d     Consensus state:  %s\n", round, concurrency_block_numbers, state ? "running" : "rest");
    ...
    printf("\n=============== [PHASE VALIDATE WAITING QUEUE: ]   Waiting to Vote:  %ld\n", bc->waiting_for_phase_1_block.size());

    unsigned long blocks = 0;
    for (auto it : bc->total_ask_for_verify_blocks)
    {
        blocks += it.second;
    }
    unsigned long txs = blocks * TX_NUMBERS_IN_A_BLOCK * 0.22 * 1024;
    printf("\n=============== [VALIDATE BLOCKS THROUGHPUT: ]    txs MB/s:  %.2f  txs GB/h:  %.1f \n", txs / (1024.0 * 1024) / secs, txs / (1024.0 * 1024) / secs * 3600 / 1000);

    if (!consensus_time.empty())
    {
        float ave_consensus_time = 0;
        float total_consensus_time = 0;
        for (auto it : consensus_time)
        {
            total_consensus_time += it.second;
        }
        ave_consensus_time = total_consensus_time / consensus_time.size();

        printf("\n=============== [AVERAGE CONSENSUS TIME: ]    Time:   %0.2f\n", ave_consensus_time);
    }
}
