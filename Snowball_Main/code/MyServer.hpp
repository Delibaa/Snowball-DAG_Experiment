...
typedef struct Peers{
    ...
    deque <string> _m;//客户端发送消息到该节点，先把message暂时存储在这里
...
public:
    ...
    void send_verify_block_to_peers(network_block *nb);
    ...
    void send_verify_block_to_one_peer(string sender_ip, uint32_t sender_port, uint32_t chain_id, BlockHash parent, BlockHash hash, block *b );
    ...
    string get_transaction_pool_folder();
...
private:
  ...
  string folder_transaction_pool;
};
